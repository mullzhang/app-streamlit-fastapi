...
def optimize(numbers: List[int], num_partitions: int, job_id: str):
    ...
    variance = sum(
        (sum(number_i * x[i, p] for i, number_i in enumerate(numbers)) - target) ** 2
        for p in range(num_partitions)
    )
    onehot = sum(
        (sum(x[i, p] for p in range(num_partitions)) - 1) ** 2
        for i in range(len(numbers))
    )
    cost_func = variance / num_partitions / max(numbers) + Placeholder(
        "onehot"
    ) * Constraint(onehot, "onehot")
    ...
    sampleset = (
        ...
        .sample(bqm, num_reads=10, num_sweeps=10000)
        .aggregate()
    )
    ...
    if len(feasible_samples) < 1:
        ...
        results = dict(
            solutions=[
                dict(partition=partition, evaluation=evaluation)
